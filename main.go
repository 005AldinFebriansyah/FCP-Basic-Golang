...
func (sm *InMemoryStudentManager) Login(id string, name string) (string, error) {
	for _, s := range sm.students {
		if s.ID == id && s.Name == name {
			return fmt.Sprintf("Login berhasil: %s", s.Name), nil
		}
	}
	return "", fmt.Errorf("Login failed: Invalid id or name") // TODO: replace this
}
...
func (sm *InMemoryStudentManager) Register(id string, name string, studyProgram string) (string, error) {
	...
	return fmt.Sprintf("Registrasi berhasil: %s (%s)", newStudent.Name, newStudent.StudyProgram), nil // TODO: replace this
}

func (sm *InMemoryStudentManager) GetStudyProgram(code string) (string, error) {
	if sp, ok := sm.studentStudyPrograms[code]; ok {
		return sp, nil
	}
	return "", fmt.Errorf("Invalid study program code") // TODO: replace this
}
...
func (sm *InMemoryStudentManager) ModifyStudent(name string, fn model.StudentModifier) (string, error) {
	...
	return "", fmt.Errorf("Student not found") // TODO: replace this
}
...
func (sm *InMemoryStudentManager) ChangeStudyProgram(programStudi string) model.StudentModifier {
	return func(s *model.Student) error {
		if _, err := sm.GetStudyProgram(programStudi); err != nil {
			return err
		}
